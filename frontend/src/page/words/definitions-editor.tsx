...
function DefinitionRow({
  ...
  useEffect(() => {
    setLocalDef(item.definition);
  }, [item.definition]);
  useEffect(() => {
    setLocalCtx(item.context);
  }, [item.context]);
  ...
  return (
    ...
    >
      {isEdit && (
        <input type="hidden" name="definition_ids[]" value={item.id} />
      )}
...
function DefinitionsEditor({ initialItems, isEdit }: DefinitionsEditorProps) {
  ...
  const handleRemoveClick = () => {
    ...
    const hasContent =
      selectedItem.definition.trim() || selectedItem.context.trim();
    if (hasContent) {
    ...
    <>
          ...
          {items.map((item) => (
            <DefinitionRow
              ...
              onSelect={() =>
                dispatch({
                  type: "Select",
                  key: item.key === selectedKey ? null : item.key,
                })
              }
              onChange={(def, ctx) =>
                dispatch({
      ...
      <ModalInner
        ...
        contents={(close) => (
          <>
            <p>
              this definition has content. are you sure you want to remove it?
            </p>
            ...
            <div className="help-content">
              <section>
                ...
                <dl className="keybind-list">
                  <Keybind
                    keys={["Ctrl", "↑"]}
                    description="Move selected row up"
                  />
                  <Keybind
                    keys={["Ctrl", "↓"]}
                    description="Move selected row down"
                  />
                  <Keybind
                    keys={["Ctrl", "Enter"]}
                    description="Add row after selected"
                  />
                </dl>
              </section>
              <section>
...
export function mountDefinitionsEditor(
  ...
  root.render(
    <DefinitionsEditor initialItems={itemsWithKeys} isEdit={options.isEdit} />,
  );
}
