...
import type {
  QuotationPossiblyNew,
  QuotationsEditorElement,
  QuotationWithWordInfo,
  TextElement,
} from "./types";
import { ControlButton } from "../languages/phonology-editor/controls";
import {
  Editable,
  Slate,
  withReact,
    ...
    <div className="highlight-picker-wrapper">
      ...
      <div className="highlight-picker-actions">
        <button
          ...
          disabled={!highlight}
          onClick={() => onHighlightChange(null, null)}
        >
          clear
        </button>
        <button
...
type Action =
  ...
  | {
      ...
      quotation: QuotationPossiblyNew | null;
      edge?: "before" | "after";
    };
...
const applyAction = (state: EditorState, action: Action): EditorState => {
  switch (action.type) {
    ...
    case "OpenEditModal":
      return {
        ...state,
        pendingModal: "EditQuotation",
        editingQuotation: action.quotation,
      };
    case "OpenRemoveModal":
      return {
        ...state,
        pendingModal: "RemoveQuotation",
        removingQuotation: action.quotation,
      };
    ...
    case "SelectQuotation":
      return {
        ...
        selectionEdge: action.edge ?? null,
      };
    default:
      return state;
  }
...
const EditorContext = createContext<[EditorState, React.Dispatch<Action>]>(
  null!,
);
...
const addQuotation = (
  ...
  selectedDefinition: DefinitionOption | null,
  selection: Range | null,
  editor: Editor,
  highlight: Highlight | null,
  notes: string,
...
const loadDefinitions =
  (
    word: WordOption,
    languageCode: string,
    setDefinitionsError: (error: string) => void,
  ) =>
  async (input: string): Promise<DefinitionOption[]> => {
    try {
      const response = await fetch(
        `/api/languages/${languageCode}/words/${word.slug}/${word.lemma}/definitions`,
      );
      if (!response.ok) {
      ...
      return definitions.filter((def) =>
        def.definition.toLowerCase().includes(input.toLowerCase()),
      );
    } catch (e) {
...
const AddQuotationModal = ({
  ...
  const [selectedDefinition, setSelectedDefinition] =
    useState<DefinitionOption | null>(null);
        ...
        >
          {quotationText && (
            <section>
              ...
              <p className="hint">
                select the part of the text which represents the word you want
                to link to
              </p>
              <HighlightPicker
                text={quotationText}
          ...
          <section>
            ...
            <WordCombobox
              ...
              initialSearch={
                selectedWord ? undefined : (highlightText ?? undefined)
              }
            />
          </section>
          ...
          {selectedWord ? (
            <section>
              ...
              {definitionsError && (
                <div className="error">{definitionsError}</div>
              )}
              <AsyncSelect<DefinitionOption, false>
                inputId="add-quotation-definition"
                cacheOptions
                defaultOptions
                loadOptions={loadDefinitions(
                  selectedWord,
                  languageCode,
                  setDefinitionsError,
                )}
              ...
              <span className="hint">
                ...
                <a
                  href={`/languages/${languageCode}/words/${selectedWord.slug}/${selectedWord.lemma}/edit`}
                >
                  edit this word
                </a>
              </span>
          ...
          <div className="button-row">
            <button className="secondary normal" type="button" onClick={close}>
              Cancel
            </button>
            <button
...
const EditQuotationModal = ({
  ...
  const [selectedDefinition, setSelectedDefinition] =
    useState<DefinitionOption | null>(() => ({
  ...
  const quotationText = Array.from(
    Editor.nodes<TextElement>(editor, {
      ...
      match: (n) =>
        !Editor.isEditor(n) &&
        !Element.isElement(n) &&
        (n as TextElement).quotation === quotation,
    }),
  )
    .map(([node]) => (node as TextElement).text)
    .join("");
        ...
        >
          {quotationText && (
            <section>
              <label>Highlight</label>
              <p className="hint">
                Select part of the text to highlight it and pre-fill the word
                search.
              </p>
              <HighlightPicker
                text={quotationText}
...
const RemoveQuotationModal = ({
  open,
  setOpen,
  quotation,
  onConfirm,
}: RemoveQuotationModalProps) => {
  return (
    <ModalInner
      open={open}
      close={() => setOpen(false)}
      ...
      contents={(close) => (
        <div>
          <p>
            Remove the quotation for <strong>{quotation.word}</strong>?
          </p>
...
const QuotationsEditorControls = ({
  ...
  const openEditQuotation = () => {
    if (editorState.selectedQuotation) {
      dispatch({
        type: "OpenEditModal",
        quotation: editorState.selectedQuotation,
      });
    }
  };
  ...
  const atLeftEdge =
    quotationsCount === 0 ||
    (editorState.selectedQuotation === null &&
      editorState.selectionEdge === "before");
  const atRightEdge =
    quotationsCount === 0 ||
    (editorState.selectedQuotation === null &&
      editorState.selectionEdge === "after");
  const selectionCoversQuotation =
    ...
    Array.from(
      Editor.nodes<TextElement>(editor, {
        ...
        match: (n) =>
          !Editor.isEditor(n) &&
          !Element.isElement(n) &&
          !!(n as TextElement).quotation,
      }),
    ).length > 0;
  const canRemove =
    editorState.selectedQuotation !== null || selectionCoversQuotation;
    ...
    <>
      <div className="controls-container">
        ...
        <div className="controls quotation-controls">
          ...
          <ControlButton
            onClick={openAddQuotation}
            title="Add at selection"
          ...
          <ControlButton
            onClick={openEditQuotation}
            title="Edit quotation"
            enabled={editorState.selectedQuotation != null}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1em"
            ...
            <div className="help-content">
              ...
              <section>
                ...
                <dl className="keybind-list">
                  <Keybind
                    keys={["Ctrl", "k"]}
                    description="Add quotation at selection"
                  />
                </dl>
              </section>
            </div>
...
const Leaf = (props: RenderLeafProps) => {
  ...
  const isSelected =
    props.leaf.quotation != null &&
    props.leaf.quotation === editorState.selectedQuotation;
  ...
  if (props.leaf.quotation) {
    ...
    if (
      props.leaf.quotation.highlight_start !== null &&
      props.leaf.quotation.highlight_end !== null
    ) {
...
const initialValue = (
  props: QuotationsEditorProps,
): QuotationsEditorElement[] => {
  const parsed = props.quotations.reduce(
    (acc, quotation) => {
      const beforeText = props.text.slice(acc[1], quotation.span_start);
      const quotationText = props.text.slice(
...
const QuotationsEditor = (props: QuotationsEditorProps) => {
  ...
  const handleEditSave = (
    oldQuotation: QuotationPossiblyNew,
    updatedQuotation: QuotationPossiblyNew,
  ) => {
    const nodeEntries = Array.from(
      Editor.nodes(editor, {
        ...
        match: (n) =>
          !Editor.isEditor(n) &&
          !Element.isElement(n) &&
          (n as TextElement).quotation === oldQuotation,
      }),
    );
    for (const [, path] of nodeEntries) {
      Transforms.setNodes<TextElement>(
        editor,
  ...
  const handleRemoveQuotation = () => {
    ...
    if (selection && !Range.isCollapsed(selection)) {
      ...
      const wouldRemoveEntire = allQuotationNodes.every(([node, path]) => {
        ...
        return (
          Range.includes(selection, nodeStart) &&
          Range.includes(selection, nodeEnd)
        );
      });
  ...
  return (
    <EditorContext.Provider value={[editorState, dispatch]}>
      <section className="quotations-editor">
        <label id="quotation-editor-label" className="editor-label">
          Translated text
        </label>
        {props.error ? (
        ...
        />
        <Slate editor={editor} initialValue={initial} onChange={onSlateChange}>
          <Editable
        ...
        <AddQuotationModal
          ...
          setOpen={(open) => {
            if (!open) dispatch({ type: "ClearPendingModal" });
          }}
          languageCode={props.languageCode}
        />
        {editorState.editingQuotation && (
          <EditQuotationModal
            ...
            onSave={(updated) =>
              handleEditSave(editorState.editingQuotation!, updated)
            }
          />
        )}
        {editorState.removingQuotation && (
          <RemoveQuotationModal
            ...
            onConfirm={() =>
              handleConfirmRemove(editorState.removingQuotation!)
            }
          />
        )}
      </section>
