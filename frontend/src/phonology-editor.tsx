...
const PhonologyEditor = ({ body, name }: { body: Body; name: string }) => {
  ...
  const className = "phonology-table" + (state.focusInsideTable ? " focus-inside-table" : "");
    ...
    <EditorContext.Provider value={[state, dispatch]}>
      <div className="phonology-editor">
        ...
        <div className="header-with-actions">
          <h2>{name}</h2>
          <ul>
            <li><Help open={helpOpen} setOpen={setHelpOpen} /></li>
          </ul>
        </div>
        <table className={className} onKeyDown={handleKeyPress} onFocus={onFocusIn} onBlur={onFocusOut} ref={tableRef}>
          <thead>
            <ColumnHeaders />
          </thead>
          <tbody>
