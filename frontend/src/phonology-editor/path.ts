...
import { elementKind, headingChildren, numLeaves, TOP_LEFT_CELL } from "./table";
...
export type TablePath = TopLeftPath | RowHeadingPath | ColumnHeadingPath | CellPath;

export type PathTarget<T extends TablePath> =
  T extends TopLeftPath ? typeof TOP_LEFT_CELL :
  T extends RowHeadingPath ? Row :
  T extends ColumnHeadingPath ? Column :
  T extends CellPath ? Cell :
  TableElement;
 ...
 * @param path 
 */
export const headingPathToIndex = (headings: Heading[], path: HeadingPath): number | null => {
    let index = 0;
    ...
    for (const pathIndex of path) {
        ...
        index += currentHeadings.slice(0, pathIndex).reduce((sum, heading) => sum + countLeaves(heading), 0);
        currentHeadings = headingChildren(currentHeading);
    }
    return index;
}

export const indexToHeadingPath = (headings: Heading[], targetIndex: number): HeadingPath | null => {
    let index = 0;
    let path: number[] = [];
    let currentHeadings = headings;
    while (true) {
...
const stepHeadingPath = <T extends Row | Column>(current: T, index: number): T | null => {
...
const followHeadingPath = <T extends Row | Column>(headings: T[], path: HeadingPath): T | null => {
...
export const normalizeHeadingPath = (headings: Heading[], path: HeadingPath | number): HeadingPath | null => {
...
const normalizeToIndex = (headings: Heading[], path: HeadingPath | number): number | null => {
    if (typeof path === "number") {
        return path;
    }
...
export const isPathPrefixed = (prefix: HeadingPath, path: HeadingPath): boolean => {
...
export const isPathEqual = (table: Body, path1: TablePath, path2: TablePath): boolean => {
    if (path1.type === path2.type) {
        switch (path1.type) {
            ...
            case "RowHeading": {
                ...
                const normalizedPath2 = normalizeHeadingPath(table.rows, (path2 as RowHeadingPath).path);
                if (normalizedPath1 === null || normalizedPath2 === null) {
                    return false;
                }
                return JSON.stringify(normalizedPath1) === JSON.stringify(normalizedPath2);
            }
            case "ColumnHeading": {
                const normalizedColPath1 = normalizeHeadingPath(table.columns, path1.path);
                const normalizedColPath2 = normalizeHeadingPath(table.columns, (path2 as ColumnHeadingPath).path);
 ...
 * @returns 
 */
export const getByPath = <T extends TablePath>(table: Body, path: T): PathTarget<T> | null => {
  switch (path.type) {
    case "TopLeft":
...
const setByPathInHeadings = <T extends Row | Column>(
    ...
    return headings.map((heading, i) => {
        ...
        if (heading.type === "Group") {
            if ("rows" in heading) {
                return { ...heading, rows: setByPathInHeadings(heading.rows as T[], rest, newValue) };
            } else {
                return { ...heading, columns: setByPathInHeadings((heading as any).columns as T[], rest, newValue) };
            }
        }
        return heading;
    });
...
const setCellInRow = (rows: Row[], rowPath: HeadingPath, colIndex: number, newValue: Partial<Cell>): Row[] => {
...
export const setByPath = <T extends TablePath>(table: Body, path: T, newValue: Partial<PathTarget<T>>): Body => {
    switch (path.type) {
        case "TopLeft":
            return table;
        case "RowHeading": {
            ...
            const newRows = setByPathInHeadings(table.rows, normalizedPath, newValue as Partial<Row>);
        ...
        case "ColumnHeading": {
            ...
            const newColumns = setByPathInHeadings(table.columns, normalizedPath, newValue as Partial<Column>);
        ...
        case "Cell": {
            ...
            const newRows = setCellInRow(table.rows, normalizedRowPath, colIndex, newValue as Partial<Cell>);
            return { ...table, rows: newRows };
        }
    }
}
...
const headingParent = (headings: Heading[], target: HeadingPath): HeadingPath | null => {
    const normalized = normalizeHeadingPath(headings, target);
    if (normalized === null || normalized.length === 0 || normalized.length === 1) {
        return null;
    }
    return normalized.slice(0, -1);
}

const headingFirstChild = (headings: Heading[], target: HeadingPath): HeadingPath | null => {
    const normalized = normalizeHeadingPath(headings, target);
    if (normalized === null) {
        return null;
    }
...
const adjacentInList = (paths: HeadingPath[], target: HeadingPath, direction: "Next" | "Previous"): HeadingPath | null => {
...
const collectPathsAtDepth = (headings: Heading[], depth: number, prefix: HeadingPath = []): HeadingPath[] => {
...
const collectPathsDFS = (headings: Heading[], prefix: HeadingPath = []): HeadingPath[] => {
    const paths: HeadingPath[] = [];
    for (let i = 0; i < headings.length; i++) {
        const path = [...prefix, i];
        paths.push(path);
 ...
 * @param direction
 */
const headingSiblingByDepth = (headings: Heading[], target: HeadingPath, direction: "Next" | "Previous"): HeadingPath | null => {
    return adjacentInList(collectPathsAtDepth(headings, target.length), target, direction);
}
 ...
 * @param direction
 */
const headingSiblingByTree = (headings: Heading[], target: HeadingPath, direction: "Next" | "Previous"): HeadingPath | null => {
 ...
 * @param movement 
 */
export const move = (table: Body, path: TablePath, movement: Movement): TablePath | null => {
    ...
    switch (elementKind(currentElement)) {
        ...
        case "RowHeading": {
            ...
            const rowPath = normalizeHeadingPath(table.rows, (path as RowHeadingPath).path);
            if (rowPath === null) {
                return path;
            }
            ...
            if (isLeaf && (movement === "Right" || movement === "Tab")) {
                if (row.cells.length > 0) {
                    return { type: "Cell", rowPath: (path as RowHeadingPath).path, colPath: 0 };
                }
            }
            ...
            if (movement === "Tab") {
                ...
                const prevSiblingPath = headingSiblingByTree(table.rows, rowPath, "Previous");
                if (prevSiblingPath !== null) {
                    return { type: "RowHeading", path: prevSiblingPath };
                }
                return path;
            } else if (movement === "Down") {
                const nextSiblingPath = headingSiblingByDepth(table.rows, rowPath, "Next");
                if (nextSiblingPath !== null) {
                    return { type: "RowHeading", path: nextSiblingPath };
                }
                return path;
            } else if (movement === "Up") {
                const prevSiblingPath = headingSiblingByDepth(table.rows, rowPath, "Previous");
                if (prevSiblingPath !== null) {
                    return { type: "RowHeading", path: prevSiblingPath };
                }
                return path;
        ...
        case "ColumnHeading": {
            ...
            const colPath = normalizeHeadingPath(table.columns, (path as ColumnHeadingPath).path);
            if (colPath === null) {
                return path;
            }
            ...
            if (isLeaf && (movement === "Down")) {
                if (table.rows.length > 0) {
                    return { type: "Cell", rowPath: 0, colPath: (path as ColumnHeadingPath).path };
                }
            }
            ...
            // all special cases handled; now handle siblings
            if (movement === "Tab") {
                const nextSiblingPath = headingSiblingByTree(table.columns, colPath, "Next");
                if (nextSiblingPath !== null) {
                    return { type: "ColumnHeading", path: nextSiblingPath };
                }
                return path;
            } else if (movement === "ShiftTab") {
                const prevSiblingPath = headingSiblingByTree(table.columns, colPath, "Previous");
                if (prevSiblingPath !== null) {
                    return { type: "ColumnHeading", path: prevSiblingPath };
                }
                return path;
            } else if (movement === "Right") {
                const nextSiblingPath = headingSiblingByDepth(table.columns, colPath, "Next");
                if (nextSiblingPath !== null) {
                    return { type: "ColumnHeading", path: nextSiblingPath };
                }
                return path;
            } else if (movement === "Left") {
                const prevSiblingPath = headingSiblingByDepth(table.columns, colPath, "Previous");
                if (prevSiblingPath !== null) {
                    return { type: "ColumnHeading", path: prevSiblingPath };
                }
                return path;
        ...
        case "Cell": {
            ...
            const colIndex = normalizeToIndex(table.columns, (path as CellPath).colPath);
            ...
            switch (movement) {
                ...
                case "Tab":
                    ...
                    if (!isLastCol) {
                        ...
                        // of the tree
                        const nextRowHeadingPath = headingSiblingByTree(table.rows, normalizeHeadingPath(table.rows, (path as CellPath).rowPath)!, "Next");
                ...
                case "Home":
                    return { type: "RowHeading", path: (path as CellPath).rowPath };
                case "End":
                    const rowHeading = followHeadingPath(table.rows, normalizeHeadingPath(table.rows, (path as CellPath).rowPath)!);
                    if (rowHeading === null) {
                        return path;
                    }
                    const lastCellIndex = lastCellOfRow(rowHeading);
                    if (lastCellIndex !== null) {
                        return { type: "Cell", rowPath: (path as CellPath).rowPath, colPath: lastCellIndex };
                    } else {
                        return path;
                    }
            }
