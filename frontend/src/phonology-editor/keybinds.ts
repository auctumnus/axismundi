...
export const getKeybindAction = (state: EditorState, e: KeyboardEvent): Action | null => {
    const targetPath =
                ...
                switch(targetPath.type) {
                    case "Cell":
                        ...
                        if (state.keybindState === "Annotation") {
                            const cellHasAnnotation = cell.phonemes.some(p => p.annotations.length > 0);
                            const modal = cellHasAnnotation ? "EditAnnotation" : "AddAnnotation";
                            return { type: "OpenModal", modal };
                        }
                        return { type: "SetKeybindState", keybindState: "Idle" };
                    case "RowHeading":
