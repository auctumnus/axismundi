...
import { countLeaves, getByPath, headingPathToIndex, indexToHeadingPath, move, setByPath, type TablePath } from "./path";
...
test("countLeaves base case", () => {
    const individualRow: Row = { type: "Individual", heading: "Row 1", cells: [] };
...
test("getByPath with cell path", () => {
    const table: Body = {
        rows: [
            { type: "Individual", heading: "Row 1", cells: [cell("a", "b"), cell("c")] },
...
test("setByPath with cell path updates the correct cell", () => {
    ...
    const expected: Body = {
        rows: [
            { type: "Individual", heading: "Row 1", cells: [cell("a"), cell("x", "y")] },
            { type: "Individual", heading: "Row 2", cells: [cell("c")] }
        ],
        columns: [
            { type: "Individual", heading: "Column 1" },
...
test("setByPath on row heading updates the correct row", () => {
    ...
    const result = setByPath(table, path, { type: "Individual", heading: "New Row 1", cells: [cell("x")] });
    const expected: Body = {
        rows: [
...
test("setByPath on column heading updates the correct column", () => {
    ...
    const result = setByPath(table, path, { type: "Individual", heading: "New Column 1" });
    const expected: Body = {
        rows: [
            { type: "Individual", heading: "Row 1", cells: [cell("a"), cell("b")] },
            { type: "Individual", heading: "Row 2", cells: [cell("c")] }
...
test("setByPath on row heading with groups updates the correct row", () => {
    ...
    const path: TablePath = { type: "RowHeading", path: [0] };
    // @ts-ignore
    const result = setByPath(table, path, { type: "Group", heading: "New Group 1", rows: table.rows[0]!.rows });
    const expected: Body = {
        rows: [
            {
                type: "Group",
...
test("movement: right from top-left to first column heading", () => {
    ...
    expect(move(table, { type: "TopLeft" }, "Right")).toEqual({ type: "ColumnHeading", path: 0 });
});
...
test("movement: tab from top-left to first column heading", () => {
    ...
    expect(move(table, { type: "TopLeft" }, "Tab")).toEqual({ type: "ColumnHeading", path: 0 });
});
...
test("movement: down from top-left to first row heading", () => {
    ...
    expect(move(table, { type: "TopLeft" }, "Down")).toEqual({ type: "RowHeading", path: 0 });
});
...
test("movement: end from top-left to last column heading", () => {
    ...
    expect(move(table, { type: "TopLeft" }, "End")).toEqual({ type: "ColumnHeading", path: 2 });
});
...
test("movement: home from top-level row heading goes to top-left", () => {
    ...
    expect(move(table, { type: "RowHeading", path: [1] }, "Home")).toEqual({ type: "TopLeft" });
});
...
test("movement: home from nested row heading goes to top-level parent", () => {
    const table: Body = {
        rows: [
            {
                type: "Group", heading: "Group 1",
                rows: [
                    {
                        type: "Group", heading: "Subgroup 1",
                        rows: [{ type: "Individual", heading: "Row 1", cells: [] }]
                    }
                ]
            }
    ...
    expect(move(table, { type: "RowHeading", path: [0, 0, 0] }, "Home")).toEqual({ type: "RowHeading", path: [0] });
});

test("movement: end from row heading goes to last cell", () => {
    const table: Body = {
        rows: [{ type: "Individual", heading: "Row 1", cells: [cell("a"), cell("b"), cell("c")] }],
        columns: [
    ...
    expect(move(table, { type: "RowHeading", path: [0] }, "End")).toEqual({ type: "Cell", rowPath: [0], colPath: 2 });
});
...
test("movement: right from group row heading goes to first child", () => {
    ...
    expect(move(table, { type: "RowHeading", path: [0] }, "Right")).toEqual({ type: "RowHeading", path: [0, 0] });
});
...
test("movement: up from column heading to parent", () => {
    ...
    expect(move(table, { type: "ColumnHeading", path: [0, 0] }, "Up")).toEqual({ type: "ColumnHeading", path: [0] });
});
...
test("movement: down from leaf column heading to first cell in column", () => {
    ...
    expect(move(table, { type: "ColumnHeading", path: [0] }, "Down")).toEqual({ type: "Cell", rowPath: 0, colPath: [0] });
});
...
test("movement: down from group column heading to first child", () => {
    ...
    expect(move(table, { type: "ColumnHeading", path: [0] }, "Down")).toEqual({ type: "ColumnHeading", path: [0, 0] });
});
...
test("movement: tab/shifttab through column headings", () => {
    ...
    expect(move(table, { type: "ColumnHeading", path: [0] }, "Tab")).toEqual({ type: "ColumnHeading", path: [0, 0] });
    // Tab from first child to second child
    expect(move(table, { type: "ColumnHeading", path: [0, 0] }, "Tab")).toEqual({ type: "ColumnHeading", path: [0, 1] });
    // ShiftTab back
    expect(move(table, { type: "ColumnHeading", path: [0, 1] }, "ShiftTab")).toEqual({ type: "ColumnHeading", path: [0, 0] });
});
...
test("movement: right/left through column headings by depth", () => {
    ...
    expect(move(table, { type: "ColumnHeading", path: [0] }, "Right")).toEqual({ type: "ColumnHeading", path: [1] });
    expect(move(table, { type: "ColumnHeading", path: [1] }, "Left")).toEqual({ type: "ColumnHeading", path: [0] });
});
...
test("movement: left from first column heading (all zeros) goes to top-left", () => {
    ...
    expect(move(table, { type: "ColumnHeading", path: [0, 0] }, "Left")).toEqual({ type: "TopLeft" });
});
...
test("movement: left from non-first column heading by depth", () => {
    ...
    expect(move(table, { type: "ColumnHeading", path: [0, 1] }, "Left")).toEqual({ type: "ColumnHeading", path: [0, 0] });
});
...
test("movement: up from first row cell to column heading", () => {
    ...
    expect(move(table, { type: "Cell", rowPath: 0, colPath: 1 }, "Up")).toEqual({ type: "ColumnHeading", path: 1 });
});
...
test("movement: tab from non-last col cell moves right", () => {
    const table: Body = {
        rows: [{ type: "Individual", heading: "Row 1", cells: [cell("a"), cell("b")] }],
        columns: [
            { type: "Individual", heading: "Col 1" },
            { type: "Individual", heading: "Col 2" }
        ],
    ...
    expect(move(table, { type: "Cell", rowPath: 0, colPath: 0 }, "Tab")).toEqual({ type: "Cell", rowPath: 0, colPath: 1 });
});
...
test("movement: shifttab from first col cell goes to row heading", () => {
    ...
    expect(move(table, { type: "Cell", rowPath: 0, colPath: 0 }, "ShiftTab")).toEqual({ type: "RowHeading", path: 0 });
});
...
test("movement: tab from last col wraps to next row heading", () => {
    ...
    expect(move(table, { type: "Cell", rowPath: 0, colPath: 0 }, "Tab")).toEqual({ type: "RowHeading", path: [1] });
});
...
test("movement: tab from last col of last row returns null (no next row)", () => {
    ...
    expect(move(table, { type: "Cell", rowPath: 0, colPath: 0 }, "Tab")).toBeNull();
});
...
test("movement: home from cell goes to row heading", () => {
    ...
    expect(move(table, { type: "Cell", rowPath: 0, colPath: 1 }, "Home")).toEqual({ type: "RowHeading", path: 0 });
});
...
test("movement: end from cell goes to last cell in row", () => {
    ...
    expect(move(table, { type: "Cell", rowPath: 0, colPath: 0 }, "End")).toEqual({ type: "Cell", rowPath: 0, colPath: 2 });
});
...
test("movement: end to last cell in row", () => {
    const table: Body = {
        rows: [
            { type: "Individual", heading: "Row 1", cells: [cell("a"), cell("b"), cell("c")] }
        ],
        columns: [
            { type: "Individual", heading: "Column 1" },
            { type: "Individual", heading: "Column 2" },
