...
export const ControlButton = ({
  ...
  return (
    <Tooltip content={title}>
      <button
        type="button"
        className={`control-button ${enabled ? "" : "disabled"}`}
        onClick={onClick}
        disabled={!enabled}
      >
        {children}
        ...
        <>
          <section>
            ...
            <input
              ...
              autoFocus
              onKeyDown={(e) =>
                e.key === "Enter" && newHeading.trim() && handleSave()
              }
            />
          </section>
...
const RowControls = ({ isRowSelected, selectTarget }: ControlState) => {
  ...
  const groupSelected =
    isRowSelected && selectTarget && "rows" in (selectTarget as Row);
  ...
  const selectedRow =
    isRowSelected && state.select?.type === "RowHeading"
    ...
    <div className="controls row-controls">
      ...
      >
        ...
        >
          ...
          <path
            fill="currentColor"
            d="M5 21q-.825 0-1.412-.587T3 19v-4q0-.825.588-1.412T5 13h14q.825 0 1.413.588T21 15v4q0 .825-.587 1.413T19 21zm0-10q-.825 0-1.412-.587T3 9V5q0-.825.588-1.412T5 3h14q.825 0 1.413.588T21 5v4q0 .825-.587 1.413T19 11zm14-6H5v4h14zM5 9V5z"
          />
        </svg>
      </ControlButton>
      <ControlButton
        onClick={deleteRow}
        title="Delete row"
        enabled={isRowSelected && !onlyOneRow}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="1em"
...
const ColumnControls = ({ isColumnSelected, selectTarget }: ControlState) => {
  ...
  const groupSelected =
    isColumnSelected && selectTarget && "columns" in (selectTarget as Column);
  ...
  const selectedColumn =
    isColumnSelected && state.select?.type === "ColumnHeading"
  ...
  return (
    <div className="controls row-controls">
      ...
      <ControlButton
        ...
        title="Delete column"
        enabled={isColumnSelected && !onlyOneColumn}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="1em"
...
const PhonemeEditModal = ({
  kind,
  phonemeIndex,
  onSave,
  onClose,
  open,
}: PhonemeEditModalProps) => {
  const [state, dispatch] = useEditor();

  const cell =
    state.select && state.select.type === "Cell"
        ...
        <>
          <section>
            ...
            <input
              ...
              autoFocus
              onKeyDown={(e) =>
                e.key === "Enter" && newPhoneme.trim() && onSave(newPhoneme)
              }
            />
          </section>
          <div className="button-row">
            ...
            <button
              type="button"
              className="normal"
              onClick={() => onSave(newPhoneme)}
              disabled={!newPhoneme.trim()}
            >
              Save
            </button>
          </div>
        ...
        <section className="phoneme-options">
          {cell?.phonemes.map((phoneme, index) => (
            <button
              ...
              onClick={() => handlePhonemeSelected(index)}
            >
              {phoneme.text}
            </button>
          ))}
...
const AnnotationControls = ({ isCellSelected, selectTarget }: ControlState) => {
  ...
  const hasMultiplePhonemes = cell !== null && cell.phonemes.length > 1;
  const selectedHasAnnotation =
    isCellSelected &&
    (selectTarget as Cell).phonemes.some((p) => p.annotations.length > 0);
    ...
    if (modalPurpose === "add") {
      ...
      if (phoneme.annotations.length === 0) {
        ...
        dispatch({
          ...
          path: state.select!,
          phonemeIndex: index,
          annotationIndex: phoneme.annotations[0]!,
        });
        closeModal();
      } else {
        setModalStep(1);
  ...
  const handleAnnotationSelected = (annotationIndex: number) => {
    if (modalPurpose === "delete") {
      dispatch({
        type: "RemoveAnnotation",
        path: state.select!,
        phonemeIndex: selectedPhonemeIndex,
        annotationIndex,
      });
      closeModal();
    } else if (modalPurpose === "edit") {
  ...
  const middleStep: SteppedModalStep = (() => {
    if (modalPurpose === "add") {
          ...
          <>
            <section>
              ...
              <input
                ...
                autoFocus
                onKeyDown={(e) =>
                  e.key === "Enter" && annotationText.trim() && handleAddSave()
                }
              />
            </section>
            <div className="button-row">
            ...
            <section className="phoneme-options">
              {availableAnnotations.map(({ text, index }) => (
                <button
                  key={index}
                  type="button"
                  className="normal phoneme-option annotation-option"
                  onClick={() => handleLinkSelected(index)}
                >
                  {index + 1}. {text}
                </button>
              ))}
              {availableAnnotations.length === 0 && (
                <p>no annotations available to link</p>
              )}
            </section>
            <div className="button-row">
              <button
      ...
      const phoneme = cell?.phonemes[selectedPhonemeIndex];
      const annotations =
        phoneme?.annotations.map((i) => ({
          text: state.body.annotations[i]!,
          index: i,
        })) ?? [];

      return {
        title:
          modalPurpose === "edit" ? "select annotation" : "delete annotation",
        content: (close: () => void) => (
          <>
            <section className="phoneme-options">
              {annotations.map(({ text, index }) => (
                <button
                  ...
                  type="button"
                  className="normal phoneme-option annotation-option"
                  onClick={() => handleAnnotationSelected(index)}
                >
                  {index + 1}. {text}
                </button>
              ))}
      ...
      <>
        <section>
          ...
          <input
            ...
            autoFocus
            onKeyDown={(e) =>
              e.key === "Enter" && annotationText.trim() && handleEditSave()
            }
          />
        </section>
        <div className="button-row">
  ...
  return (
    <div className="controls annotation-controls">
      ...
      <ControlButton
        ...
        title="Link annotation"
        enabled={hasPhonemes && hasTableAnnotations}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="1em"
          ...
          <section className="preset-options">
            {presetNames.map((name) => (
              <button
                key={name}
                type="button"
                className="normal preset-option"
...
const PresetControls = () => {
  ...
  return (
    <div className="controls preset-controls">
      ...
      <ControlButton
        onClick={() => dispatch({ type: "OpenModal", modal: "LoadPreset" })}
        enabled={true}
        title="Load preset"
      >
        <svg
