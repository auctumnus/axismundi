...
const Phoneme = ({
  ...
  return (
    <>
      ...
      {annotations.map((index, i) => (
        <>
          <sup key={index} className="annotation-link">
            {index + 1}
...
const RowCell = ({
  ...
  const colFocused = isFocused(state, {
    type: "ColumnHeading",
    path: path.colPath,
  });
    ...
    >
      {cell.phonemes.map((p, i) => (
        <Phoneme
          ...
          isLast={i === cell.phonemes.length - 1}
          onDoubleClick={() => {
            dispatch({ type: "SetFocus", path });
            dispatch({ type: "SetSelect", path });
            dispatch({
...
type FlatRow = {
  thCells: ThCell[];
  leaf: { heading: string; cells: Cell[] };
  path: HeadingPath;
};
...
const flattenRows = (
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (row.type === "Group") {
      ...
      children[0]!.thCells.unshift({
        heading: row.heading,
        rowSpan: numLeaves(row.rows),
        colSpan: 1,
        path: rowPath,
      });
...
const Th = ({ heading, rowSpan, colSpan, path }: ThCell) => {
  ...
  return (
    <th
      ...
      onKeyDown={onKeyDown}
      tabIndex={tabIndex}
      data-path={serializePath(state.body, { type: "RowHeading", path })}
    >
      {heading}
    </th>
  );
...
export const TableRows = () => {
  ...
  return (
    <>
      {flat.map((flatRow, i) => {
        const rowFocused = isFocused(state, {
          type: "RowHeading",
          path: flatRow.path,
        });
        return (
