...
const Toolbar = () => {
  ...
  const hasAnnotationInSelection =
    CustomEditor.hasAnnotationInSelection(editor);

  const handleAnnotationClick = () => {
    // Prevent overlapping annotations
    if (hasAnnotationInSelection) {
      setErrorMessage(
        "The selection contains already annotated text. Remove the existing annotation first.",
      );
      return;
    }
...
export const AnnotationEditor = () => {
  ...
  return (
    <Slate editor={editor} initialValue={initialValue}>
      <Toolbar />
      <Editable renderLeaf={renderLeaf} className="editor-editable" />
    </Slate>
  );
};
