...
function UserCombobox({
  ...
  const loadOptions = async (inputValue: string): Promise<OptionsOrGroups<UserOption, GroupBase<UserOption>>> => {
    if (!inputValue || inputValue.length === 0) {
      return []
    }

    try {
      const params = new URLSearchParams({ text_query: inputValue, limit: '20' })
