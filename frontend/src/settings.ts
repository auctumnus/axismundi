...
const lightBackgroundPage = [210, 0.40, 0.96] as const // hsl(210, 40%, 96%)
...
private hueValue: HTMLInputElement | null = null;
  ...
  private contrastWarningText(): string {
    ...
    if(contrast.light.isAccessible() && contrast.dark.isAccessible()) {
        return '';
    } else if(!contrast.light.isAccessible() && !contrast.dark.isAccessible()) {
  ...
  render() {
    const pfp = document.querySelector('#current-user') as HTMLImageElement | null;
  ...
  private contrast(): { light: ContrastResult, dark: ContrastResult } | null {
    ...
    const light = contrast('light', `hsl(${this.hue}, ${this.saturation}%, ${this.lightness}%)`);
    const dark = contrast('dark', `hsl(${this.hue}, ${this.saturation}%, ${this.lightness}%)`);
    return {
        light,
        dark,
    }
  ...
  private attachElements() {
    ...
    this.lightnessSlider = document.getElementById('lightness-slider') as HTMLInputElement;

    this.hueValue = document.getElementById('hue-value') as HTMLInputElement;
    this.saturationValue = document.getElementById('saturation-value') as HTMLInputElement;
    this.lightnessValue = document.getElementById('lightness-value') as HTMLInputElement;

    this.input = this.querySelector('#gender') as HTMLInputElement;
    this.container = document.getElementById('gender-container') as HTMLDivElement;

    this.textPreview = document.getElementById('gender-text-preview') as HTMLDivElement;
  ...
  private updatePreview() {
    if(Number.isNaN(this.hue) || Number.isNaN(this.saturation) || Number.isNaN(this.lightness)) { 
        this.hue = null;
        this.saturation = null;
        this.lightness = null;
    }
    if(this.preview) {
        ...
        this.preview.innerHTML = this.value ? '' : '<span>No color selected</span>';
    }
    if(this.input) {
  ...
  private attachCallbacks() {
    ...
    const clearButton = this.querySelector('#clear-button') as HTMLButtonElement;
