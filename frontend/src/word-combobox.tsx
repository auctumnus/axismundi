...
function WordCombobox({
  ...
  const [selectedOption, setSelectedOption] = React.useState<WordOption | null>(null)

  // Load options from API
  const loadOptions = async (inputValue: string): Promise<OptionsOrGroups<WordOption, GroupBase<WordOption>>> => {
    ...
    <div className="word-combobox-option-content">
      <span className="word-combobox-word">
        ...
        {option.word_class_abbreviation && (
          <span className="word-combobox-class"> ({option.word_class_abbreviation})</span>
        )}
      </span>
  ...
  const customStyles: StylesConfig<WordOption, false, GroupBase<WordOption>> = {
    control: (base, state) => ({
      ...
      borderColor: state.isFocused ? 'var(--input-border-focus)' : 'var(--input-border)',
